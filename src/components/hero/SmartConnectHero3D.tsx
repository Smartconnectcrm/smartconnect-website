// src/components/hero/SmartConnectHero3D.tsx
"use client"
...
export type SmartConnectHero3DProps = {
  /** Animation speed multiplier (e.g. 0.45 defense, 1.15 scale) */
  speed?: number
  /**
   * Light/material intensity multiplier.
   * Drives envMapIntensity & light intensity subtly.
   */
  intensity?: number
}
...
function Scene({ speed, intensity }: { speed: number; intensity: number }) {
  ...
  const s = clampNum(speed, 0.25, 2.0)
  const env = clampNum(intensity, 0.85, 1.35)
  ...
  const materials = useMemo(() => {
    const gold = new THREE.MeshStandardMaterial({
      ...
      metalness: 0.95,
      roughness: 0.22,
      envMapIntensity: 1.0 * Number(env),
    })
    ...
    const diamond = new THREE.MeshPhysicalMaterial({
      ...
      envMapIntensity: 1.2 * Number(env),
    })
    ...
    const bgMat = new THREE.MeshStandardMaterial({
      ...
      emissiveIntensity: 0.8 * Number(env),
    })
  ...
  useFrame((_, delta) => {
    ...
    g.rotation.y += delta * 0.12 * s
    g.rotation.x += delta * 0.04 * s
    ...
    for (let i = 0; i < g.children.length; i++) {
      ...
      const def = rings[i]
      const baseSpeed = 0.7
      const tt = t * baseSpeed * s + def.phase
    ...
    if (bg.current) {
      const p = clampNum(Math.sin(t * 0.5) * 0.03, -0.03, 0.03)
  ...
  return (
    <>
      ...
      <pointLight position={[2.6, 1.8, 2.4]} intensity={1.0 * Number(lightBoost)} />
      <pointLight position={[-2.2, -1.6, 2.0]} intensity={0.8 * Number(lightBoost)} />

      <group ref={root}>
        {rings.map((r) => {
          const mat = r.id === "gold" ? materials.gold : r.id === "silver" ? materials.silver : materials.diamond
          return (
            <mesh key={r.id}>
...
export default function SmartConnectHero3D({ speed = 0.7, intensity = 1.0 }: SmartConnectHero3DProps) {
  const dpr = useAdaptiveDpr()
  const boost = clampNum(intensity, 0.85, 1.35)
  ...
  return (
    <div className="h-full w-full">
      ...
      >
        <ambientLight intensity={0.55 * Number(boost)} />
        <directionalLight position={[4, 4, 6]} intensity={1.0 * Number(boost)} />
        <Scene speed={speed} intensity={intensity} />
      </Canvas>
    </div>
  )
}
